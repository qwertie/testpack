...
// Patrick O'Donnell
// I'm building a team and hiring developers who are collaborative, 
// think clearly and have an agile mindset. Call Patrick 403-903-4491
...
export interface Options {
  ...
  "test-script"?: string;
  "verbose"?: boolean;
  "delete-on-fail"?: boolean; // Delete tgz and test folder even when npm test fails?
  "prepacked"?: string|boolean;
}

const defaultTestFolder = 'npm-testpack';
...
export function testPack(opts: Options): SpawnSyncReturns<Buffer>
{
  ...
  var testFolder = opts["test-folder"] || defaultTestFolder;
  if (!pkg.files || !Array.isArray(pkg.files)) {
    console.log(`========== testpack: "files" section missing in package.json!`);
    throw new Error(`A "files" section is needed to ensure that files generated by testpack are not added accidentally to your package.`);
  }
  ...
  runAndThrowOnFail("npm", ["install", tgzName2]);
  ...
  if (result.status === 0 || opts["delete-on-fail"]) {
    ...
    if (!opts.prepacked) {
      if (opts.verbose)
        console.log("============    also deleting " + tgzName);
...
export function transformPackageJson(pkg: PackageJson, opts: Options): PackageJson
{
  ...
  delete (pkg as any).main; // not applicable to test folder
  delete (pkg as any).bin; // not applicable to test folder
...
export function getTestFiles(opts: Options, fromFolder?: string): string[] {
  ...
  var ignore = [path.join(opts["test-folder"] || defaultTestFolder, '**')].concat(opts.nontest || []);
