...
6. Copies your test files to the new folder, preserving their original 
   directory structure.
7. Changes import/require commands in the tests to change the "local"
   prefix `.` or `..` to the package name, except when importing test 
   files, so that your tests are importing from *node_modules* instead.
   For example, if your package is called `mypkg` and your test code
   says `import {...} from "./main"`, testpack changes it to
   `import {...} from "mypkg/main"`. You should use the `"main"` option
